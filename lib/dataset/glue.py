...
class DataIOGLUE(object):
    def __init__(self, name='sst2', data_path='/mnt/petrelfs/xujingjing/wangyudong/datasets/'):
    ...
    def _load_dataset(self, a, name):
        if os.path.exists(self.data_path + name):
            return load_from_disk(os.path.join(self.data_path, name))
...
class GLUEDataset_show(Dataset):
    def __init__(self, sentences, labels, num_classes=2):
        super(GLUEDataset_show, self).__init__()
        self.sentences = sentences
        self.labels = labels
        self.set = MyDataset3([i for i in range(len(sentences))], self.sentences, self.labels)
    ...
    def __getitem__(self, item):
        return item, self.sentences[item], self.labels[item]

    def get_labeled_set(self):
        sentences = [[] for i in range(self.num_classes)]
        for i in range(len(self.sentences)):
            sentences[self.labels[i]].append([i, self.sentences[i]])
        self.labeled_set = sentences

    def __len__(self):
        return len(self.labels)

    def train_loader(self, data_set=None, batch=1, shuffle=False):
        if not data_set:
    ...
    def get_attack_set(self, size=10, reverse=1, path='./logs/record1/tr_sst_distilbert-base-uncased.txt'):
        ...
        for i in range(self.num_classes):
            ...
            if reverse:
                rank.reverse()
            for j in range(size):
                idt, sentence, label = self.set[rank[j].id]
                ids.append(idt)
                sentences.append(sentence)
                labels.append(label)
        return self.train_loader(data_set=MyDataset3(ids, sentences, labels), batch=32)
    ...
    def get_random_set(self, size=50):
        ...
        for i in range(self.num_classes):
            ...
            for j in range(size):
                ids.append(self.labeled_set[i][j][0])
                sentences.append(self.labeled_set[i][j][1])
                labels.append(i)
        return self.train_loader(data_set=MyDataset3(ids, sentences, labels), batch=32)
...
class GLUEDataset_ori(Dataset):
    ...
    def as_prompt(self, sen_lab, dict_keys, prompt_begin='', dataset_name='sst2'):
        mp = {
            'sentence': '<s>',
            'sentence1': '<s1>',
            'sentence2': '<s2>',
        ...
        label_mp = {
            'sst2': {
                0: 'negative', 1: 'positive', -1: 'sentiment'
            },
            'cola': {
                0: 'unacceptable', 1: 'acceptable', -1: 'grammar'
            },
            'mrpc': {
                0: 'not equivalent', 1: 'equivalent', -1: 'paraphrase'
            },
            'qqp': {
                0: 'not duplicate', 1: 'duplicate', -1: 'semantic'
            },
            'mnli': {
                0: 'entailment', 1: 'neutral', 2: 'contradicted', -1: 'entailment or not'
            },
            'qnli': {
                0: 'entailment', 1: 'not entailment', -1: 'entailment or not'
            },
            'rte': {
                0: 'entailment', 1: 'not entailment', -1: 'entailment or not'
            },
        ...
        for i in sen_lab:
            sentence, label = i
            if len(dict_keys) > 3:
                # prompt = prompt + mp[dict_keys[0]] + sentence[0] + \
                #          mp[dict_keys[1]] + sentence[1] + ':' + str(label) + '\n'
                # prompt = prompt + sentence[0] + '###' + sentence[1] + ':'
                # prompt = prompt + dict_keys[0] + ':"' + sentence[0] + '"\n' + \
                #          dict_keys[1] + ':"' + sentence[1] + '"\n' + label_mp[dataset_name][-1] + \
                #          ': ' + label_mp[dataset_name][label] + '\n###\n'
                prompt = prompt + dict_keys[0] + ':"' + sentence[0] + '"\n' + \
                         dict_keys[1] + ':"' + sentence[1] + '"\nLabel' + \
                         ': ' + label_mp[dataset_name][label] + '\n###\n'
                # prompt = prompt + dict_keys[0] + ':"' + sentence[0] + '"\n' + \
                ...
                # prompt = prompt + mp[dict_keys[0]] + sentence[0] + ':' + str(label) + '\n'
                # prompt = prompt + sentence[0] + ':' + str(label) + '\n'
                # prompt = prompt + dict_keys[0] + ':"' + sentence[0] + '"\n' + label_mp[dataset_name][-1] + \
                #          ': ' + label_mp[dataset_name][label] + '\n###\n'
                prompt = prompt + dict_keys[0] + ':"' + sentence[0] + '"\nLabel' + \
                         ': ' + label_mp[dataset_name][label] + '\n###\n'
                ...
                # prompt = prompt + dict_keys[0] + ':"' + sentence[0] + '"\n' + label_mp[dataset_name][-1] + \
                #          ': ' + label_mp[dataset_name][label] + '\n###\n'
                # prompt = prompt + "'" + sentence[0] + "': " + label_mp[dataset_name][label] + '\n###\n'
        return prompt
    ...
    def get_attack_set(self, size=10, reverse=1, path='./logs/record1/tr_sst_distilbert-base-uncased.txt'):
        ...
        # size_label = []
        # for i in range(self.num_classes):
        #     size_label.append(size // self.num_classes)
        # for i in range(size - ((size // self.num_classes) * self.num_classes)):
        #     j = random.randint(0, self.num_classes - 1)
        #     while size_label[j] == (size // self.num_classes) + 1:
        #         j = random.randint(0, self.num_classes - 1)
        #     size_label[j] += 1
        # print(size_label)
        for i in range(self.num_classes):
...
def get_glue(name='sst2', show=False):
    dataset = DataIOGLUE(name=name)
...
def get_glue_show(name='sst2'):
    dataset = DataIOGLUE(name=name)
    train_set = GLUEDataset_show(dataset.train_word, dataset.train_label, num_classes=dataset.num_classes)
    dev_set = GLUEDataset_show(dataset.dev_word, dataset.dev_label, num_classes=dataset.num_classes)
    test_set = GLUEDataset_show(dataset.test_word, dataset.test_label, num_classes=dataset.num_classes)
    return train_set, dev_set, test_set, dataset
